                ...
                <div className="flex gap-2 overflow-x-auto pb-2 w-full md:w-auto">
                  {['TODOS', 'EPI', 'LIMPEZA', 'UTENSÍLIOS', 'EQUIPAMENTOS'].map(cat => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase whitespace-nowrap transition-all ${activeCategory === cat ? 'bg-emerald-600 text-white shadow-md' : 'bg-gray-50 text-gray-400 hover:bg-gray-100'
                        }`}
                    >
                      {cat}
                    </button>
                ...
                {filteredCatalog.map(item => (
                  <div key={item.id} className="group p-4 bg-gray-50 rounded-2xl border border-transparent hover:border-emerald-200 hover:bg-white transition-all flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className={`p-2.5 rounded-xl ${item.category === 'EPI' ? 'bg-amber-100 text-amber-600' :
