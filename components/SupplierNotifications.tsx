...
const SCHOOL_OFFICIAL_DATA = {
  name: "ESCOLA ESTADUAL ANDRÉ ANTÔNIO MAGGI",
  cdce: "CONSELHO DELIBERATIVO DA COMUNIDADE ESCOLAR - CDCE",
  cnpj: "11.906.357/0001-50",
  address: "Rua Borba Gato, n° 80, Setor Oeste, Colíder/MT",
  director: "REZIERE DE SOUZA",
  directorCpf: "024.375.561-92"
};
          ...
          {filteredOccurrences.length > 0 ? filteredOccurrences.map(occ => {
            ...
            <div key={occ.id} className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm hover:border-emerald-200 transition-all group flex flex-col lg:flex-row gap-8 relative overflow-hidden">
                <div className={`absolute left-0 top-0 bottom-0 w-2 ${
                  occ.type === 'PRODUTO_ESTRAGADO' ? 'bg-red-500' : 'bg-amber-500'
                }`}></div>
                  ...
                  <div className="flex justify-between items-start">
                      <div>
                        <div className="flex items-center gap-3">
                            ...
                            <span className={`px-3 py-1 rounded-xl text-[8px] font-black uppercase border-2 ${
                              occ.status === 'RESOLVIDO' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-red-50 text-red-700 border-red-100 animate-pulse'
                            }`}>{occ.status}</span>
                        </div>
                ...
                <div className="lg:w-64 flex flex-col justify-center gap-3 border-t lg:border-t-0 lg:border-l border-gray-100 pt-6 lg:pt-0 lg:pl-8 no-print">
                  <button 
                    ...
                    className={`w-full py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg transition-all flex items-center justify-center gap-2 ${
                      occ.notificationSent 
                  ...
                  <div id={`notif-pdf-${occ.id}`} className="p-16 space-y-10 text-gray-900 font-sans border-[1px] border-gray-200 bg-white min-h-[297mm]">
                      ...
                      <div className="text-sm space-y-4">
                        ...
                        <p className="text-justify leading-relaxed">
                            Pelo presente instrumento, a <strong>{SCHOOL_OFFICIAL_DATA.cdce} DA {SCHOOL_OFFICIAL_DATA.name}</strong>, representada neste ato por seu gestor, vem apresentar <strong>NOTIFICAÇÃO EXTRAJUDICIAL</strong> em face de Vossa Senhoria, acerca do <strong>NÃO CUMPRIMENTO DAS OBRIGAÇÕES CONTRATUAIS</strong>, conforme detalhado a seguir:
                        </p>
                      ...
                      <div className="pt-24 grid grid-cols-1 gap-20 text-center">
                        <div className="text-center space-y-2">
                            ...
                            <div className="pt-10 border-t border-black max-w-sm mx-auto">
                              <p className="text-xs font-black uppercase">{SCHOOL_OFFICIAL_DATA.director}</p>
                              <p className="text-[9px] uppercase text-gray-500 font-bold">DIRETOR / PRESIDENTE DO CDCE</p>
                              <p className="text-[8px] text-gray-400">CPF: {SCHOOL_OFFICIAL_DATA.directorCpf}</p>
                            </div>
                        </div>
                      </div>
