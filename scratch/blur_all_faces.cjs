...
async function processBlurring() {
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    // Pass 1: SSD MobileNet detection with low threshold for high sensitivity
    let ssdDetections = await faceapi.detectAllFaces(canvas, new faceapi.SsdMobilenetv1Options({ minConfidence: 0.12 }));
    
    // Pass 2: TinyFaceDetector for extra small/side faces
    let tinyDetections = await faceapi.detectAllFaces(canvas, new faceapi.TinyFaceDetectorOptions({ scoreThreshold: 0.12 }));

    const allDetections = [...ssdDetections, ...tinyDetections];

    console.log(`[Photo ${i + 1}/${files.length}] ${file}: Anonymizing ${allDetections.length} face region(s)...`);
    ...
    allDetections.forEach(det => {
      ...
      const bw = Math.min(img.width - bx, Math.ceil(width + padX));
      const bh = Math.min(img.height - by, Math.ceil(height + padY));
      ...
      if (bw > 0 && bh > 0) {
        ...
        smallCtx.drawImage(canvas, bx, by, bw, bh, 0, 0, smallCanvas.width, smallCanvas.height);

        ctx.imageSmoothingEnabled = false;
        ctx.drawImage(smallCanvas, 0, 0, smallCanvas.width, smallCanvas.height, bx, by, bw, bh);
  ...
  console.log(`\n==================================================`);
  console.log(`✅ COMPLETED: ${files.length} photos processed.`);
  console.log(`✅ TOTAL FACES BLURRED: ${totalFacesBlurred}`);
  console.log(`==================================================\n`);
...
<head>
    <meta charset="UTF-8">
    <title>Relatório Fotográfico Escolar (Proteção de Imagem/Rostos Borrados) - E.E. André Maggi</title>
    <style>
        @page {
            size: A4 portrait;
