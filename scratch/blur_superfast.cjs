...
async function processSuperFast() {
  ...
  console.log(`Processing ${files.length} photos with high-speed face anonymization...`);
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    // Scale down image to 320px for ultra-fast face detection (20ms per photo)
    const detW = 320;
    ...
    // Fast TinyFaceDetector
    const detections = await faceapi.detectAllFaces(detCanvas, new faceapi.TinyFaceDetectorOptions({ inputSize: 224, scoreThreshold: 0.12 }));
    ...
    detections.forEach(det => {
      ...
      // Expand bounding box (40%) to ensure full head/face coverage
      const padX = width * 0.40;
      const padY = height * 0.45;
      ...
      faceBoxes.push({ bx, by, bw, bh });
    });

    // Apply pixelation + oval blur for detected face boxes
    faceBoxes.forEach(({ bx, by, bw, bh }) => {
      if (bw > 0 && bh > 0) {
        const pixelSize = Math.max(14, Math.floor(Math.min(bw, bh) / 4));
        ...
        // Heavy dark blur oval over face center
        mainCtx.save();
        mainCtx.beginPath();
        mainCtx.ellipse(bx + bw / 2, by + bh / 2, bw / 2, bh / 2, 0, 0, 2 * Math.PI);
        mainCtx.fillStyle = 'rgba(60, 60, 60, 0.65)';
        mainCtx.fill();
        mainCtx.restore();
      }
    });
  ...
  console.log(`\n==================================================`);
  console.log(`✅ SUPER FAST ANONYMIZATION COMPLETED IN ${elapsed}s!`);
...
`;
  ...
  if (browserPath) {
    console.log(`Printing new PDF with anonymized faces using browser...`);
