...
import SalaRecursosModule from './modules/SalaRecursosModule';
import ProjetoApaModule from './modules/ProjetoApaModule';
...
export type ModuleTypeExtended = 'hub' | 'merenda' | 'finance' | 'library' | 'scheduling' | 'teacher' | 'pedagogical' | 'almoxarifado' | 'patrimonio' | 'limpeza' | 'infraestrutura' | 'busca_ativa' | 'psychosocial' | 'mediacao' | 'secretariat' | 'sala_recursos' | 'projeto_apa' | 'civico_militar' | 'training' | 'educarte' | 'settings' | 'gamification';
...
const App: React.FC = () => {
  ...
  const [activeModule, setActiveModule] = useState<ModuleTypeExtended>(() => {
    try {
      ...
      const validModules: ModuleTypeExtended[] = [
        ...
        'busca_ativa', 'psychosocial', 'mediacao', 'secretariat', 'sala_recursos', 'projeto_apa', 'civico_militar', 'training', 'educarte', 'settings', 'gamification'
      ];
  ...
  // Sincronização forçada dos alunos de algumas turmas (6º ANO A, B, D, E ; 7º ANO A, B, D, E ; 8º Ano A) no localStorage
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (Array.isArray(parsed)) {
          // Remover os antigos
          const filtered = parsed.filter(s => s.Turma !== '6º ANO A' && s.Turma !== '6º ANO B' && s.Turma !== '6º ANO D' && s.Turma !== '6º ANO E' && s.Turma !== '7º ANO A' && s.Turma !== '7º ANO B' && s.Turma !== '7º ANO D' && s.Turma !== '7º ANO E' && s.Turma !== '8º ANO A');
          ...
          const novos8A = INITIAL_STUDENTS.filter(s => s.Turma === '8º ANO A');
          ...
          const current8A = parsed.filter(s => s.Turma === '8º ANO A');
          ...
          const needsUpdate8A = checkUpdate(current8A, novos8A);
          
          if (needsUpdateA || needsUpdateB || needsUpdateD || needsUpdateE || needsUpdate7A || needsUpdate7B || needsUpdate7D || needsUpdate7E || needsUpdate8A) {
            const updated = [...filtered, ...novos6A, ...novos6B, ...novos6D, ...novos6E, ...novos7A, ...novos7B, ...novos7D, ...novos7E, ...novos8A];
