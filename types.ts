...
export interface Student { id: string; name: string; birthDate: string; gender?: string; address: string; guardianName: string; contactPhone: string; registration_number: string; className: string; enrollment_date?: string; status: 'ATIVO' | 'TRANSFERIDO' | 'EVADIDO' | 'FORMADO'; documents: { type: string; status: 'ENTREGUE' | 'PENDENTE'; }[]; }
