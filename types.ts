...
export interface SchoolCelebration { 
  id: string; 
  title: string; 
  day: number; 
  month: number; 
  category: string; 
  iconType: string;
  subjectArea?: string; // Ex: 'Matemática', 'Língua Portuguesa & Literatura', 'Ciências da Natureza & Biologia', 'História', 'Geografia', 'Educação Física', 'Artes', 'Física & Química', 'Filosofia & Sociologia', 'Pedagogia', 'Línguas Estrangeiras'
  tributeMessage?: string;
  targetGroup?: 'DOCENTES' | 'SERVIDORES' | 'COMUNIDADE';
}
...
export * from './types/academicYear';
export interface Classroom { id: string; name: string; year: string; academic_year?: number; grade_level?: string; shift: Shift; teacherId: string; studentIds: string[]; schedule: { day: string; subjects: { time: string; subject: string }[]; }[]; }
