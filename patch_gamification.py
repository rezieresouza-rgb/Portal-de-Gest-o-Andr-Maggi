...
filepath = r"c:\Users\User\Projects\Portal-de-Gest-o-Andr-Maggi\modules\GamificationModule.tsx"

with open(filepath, "r", encoding="utf-8") as f:
    content = f.read()

# 1. Add StudentScore interface
interface_replacement = """interface ClassScore {
  className: string;
  totalPoints: number;
  breakdown: {
    grades: number;
...
content = re.sub(r'interface ClassScore \{.*?badges: string\[\];\n\}', interface_replacement, content, flags=re.DOTALL)

# 2. Add state for studentRankingData
state_replacement = """  const [activeTab, setActiveTab] = useState<'ranking' | 'ranking_alunos' | 'pontos_manuais' | 'auditoria'>('ranking');
...
content = re.sub(r"  const \[activeTab, setActiveTab\] = useState<'ranking' \| 'pontos_manuais' \| 'auditoria'>\('ranking'\);\n  const \[bimestreFiltro, setBimestreFiltro\] = useState<string>\('1º BIMESTRE'\);\n  const \[rankingData, setRankingData\] = useState<ClassScore\[\]>\(\[\]\);", state_replacement, content)
...
content = re.sub(r"  useEffect\(\(\) => \{\n    if \(activeTab === 'ranking'\) \{\n      calculateRanking\(bimestreFiltro\);\n    \}\n  \}, \[bimestreFiltro, activeTab\]\);", use_effect_replacement, content)

# 4. Modify calculateRanking to calculate student scores
calc_replacement = """  const calculateRanking = async (bimestre: string) => {
    setIsLoading(true);
    try {
...
content = re.sub(r"  const calculateRanking = async \(bimestre: string\) => \{.*?  \};\n\n  const handlePrint", calc_replacement + "\n\n  const handlePrint", content, flags=re.DOTALL)
...
content = re.sub(r"          <button\n            onClick=\{\(\) => setActiveTab\('ranking'\)\}\n            className=\{`px-3 py-1\.5 sm:px-4 sm:py-2 rounded-lg text-xs sm:text-sm font-bold transition-all whitespace-nowrap \$\{\n              activeTab === 'ranking' \? 'bg-white text-amber-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'\n            \}`\}\n          >\n            Ranking\n          </button>", tab_btn_replacement, content)

# 6. Add UI for ranking_alunos
ui_ranking_alunos = """          {activeTab === 'ranking_alunos' && (
...
content = re.sub(r"          \{activeTab === 'auditoria' && \(", ui_ranking_alunos + "\n\n          {activeTab === 'auditoria' && (", content)

with open(filepath, "w", encoding="utf-8") as f:
    f.write(content)

print("Patched GamificationModule.tsx successfully.")
