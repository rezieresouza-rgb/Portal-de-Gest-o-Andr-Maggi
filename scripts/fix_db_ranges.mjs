...
async function fix() {
    console.log('--- INICIANDO CORREÇÃO DE YEAR_RANGE ---');
    const { data, error } = await supabase.from('bncc_skills').select('code, year_range');
    if (error) {
        console.error('Erro ao buscar:', error.message);
        return;
    }

    console.log(`Analisando ${data.length} habilidades...`);
    const updates = [];
    ...
    data.forEach(r => {
        ...
        if (correctRange && r.year_range !== correctRange) {
            updates.push({ code: r.code, year_range: correctRange });
        }
    });

    if (updates.length === 0) {
        console.log('Nenhuma correção necessária (ou códigos não reconhecidos).');
        return;
    }

    console.log(`Corrigindo ${updates.length} linhas...`);

    // Processo em lotes de 50 para não sobrecarregar
    for (let i = 0; i < updates.length; i += 50) {
        ...
        if (upError) {
            ...
            console.log(`Lote ${i / 50 + 1} finalizado...`);
        }
    }
