...
async function resetContract002() {
    console.log('Iniciando limpeza e reset de saldos exclusivamente para os contratos de número 002/2026...');
    ...
    try {
        ...
        if (errC || !contracts || contracts.length === 0) {
            console.error('Contrato 002/2026 não encontrado:', errC);
            return;
        }

        console.log(`Encontrados ${contracts.length} contratos com o número 002/2026. Processando...`);

        for (const contract of contracts) {
            const contractId = contract.id;
            console.log(`\nContrato ID: ${contractId}`);

            // 1. Limpar `order_items` relacionados a `orders` deste contrato (Irá apagar baseando nos IDs das orders)
            ...
            // 3. Limpar `contract_events` do contrato
            ...
            // 4. Resetar `acquired_quantity` nos `contract_items` do contrato
            console.log('   Resetando quantidade adquirida dos itens (contract_items) vinculados ao contrato...');
        ...
        console.log('\nReset do contrato 002/2026 concluído com sucesso!');
    } catch (err) {
